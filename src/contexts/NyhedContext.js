...
import { jsxClosingElement, typeParameterInstantiation } from '@babel/types';
...
const NyhedContextProvider = (props) => {
    ...
    const [nyheder, setNyheder] = useState([
      ...
      //TODO -  SLET NYHED
      const sletNyhed = (id) => {
        setNyheder(nyheder.filter(n => n.nyhedid !== id)); // de nyheder der er forskellig fra id (fx 9) får lov til at være med i state (øvrige slettes)
      }
      ...
      const findNyhed = (id) =>{
        return nyheder.find(n => n.nyhedid === id); // returnerer den nyhed som har en id som matcher id som er sendt med param
      }
